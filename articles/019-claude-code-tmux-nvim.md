...
ではzellijを使った方法が紹介されていましたが、私はtmux派なのでtmuxで同様のことができるように設定してみました。最近neovimにお引っ越ししたばかりなので、こんな感じで出るのかとすごく参考になりました。

今までClaude Code上のプロンプトで直接文字を書くのが辛く、誤送信も多かったので、いったんメモ帳に書いてから送信していました。
しかしこれにより、Claude Codeから離れることなく、ポップアップ表示のNeovim上でプロンプトを書いて送信できるようになり、かなり快適になりました。
...
`~/.config/tmux/claude-prompt-edit.sh` というスクリプトを作成し、次の内容を記述します。
...
neovimのpopupから戻ったときに、Claude Code上に自動的にプロンプトが貼り付けられる挙動ではないので、少し気持ち悪いです。そのままEnter押すとちゃんと送信されるので、一応動作上は問題ありません。解決方法をご存じのほうはぜひ教えてください。
